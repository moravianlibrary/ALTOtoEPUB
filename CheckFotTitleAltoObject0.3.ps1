...
#######################
While( (($BookTitleFondSize.Maximum - $BookTitleFondSize.Average) -le 2) -and (( $BookTitleFondSize.Average - $BookTitleFondSize.Minimum ) -le 2)){
#
$BookTitleFondSize = $Global:BookTitle | measure -Maximum -Minimum -Average -Property 'Fontsize'
#remove peak values
If(($BookTitleFondSize.Maximum - $BookTitleFondSize.Average) -Ge ($BookTitleFondSize.Average - $BookTitleFondSize.Minimum)){
$Global:BookTitle=$Global:BookTitle |Where-Object { ($_.Fontsize) -eq $BookTitleFondSize.Maximum }
}
Else {$Global:BookPageNumber=$Global:BookPageNumber |Where-Object { ($_.word.string).length -match $BookTitleFondSize.Minimum }}
$Global:BookTitle=$Global:BookTitle |Where-Object { ($_.Fontsize) -eq $BookTitleFondSize.Minimum }
#
}

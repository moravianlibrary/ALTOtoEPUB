...
$Global:Book|Where-Object {($_.Type -eq "Textblock") -and (($_.word.string).Length -le 15) -and ($_.pagename -notmatch "TableOfContents")} | Sort-Object {$_.page.uuidmap.path}, LIneVPOS  | ForEach-Object -Process {
...
$Global:BookPageNumber=$Global:BookPageNumber |Where-Object { ($_.word.string).length -match $PagenumberWordLengthAverage.Maximum }
}
Else {$Global:BookPageNumber=$Global:BookPageNumber |Where-Object { ($_.word.string).length -match $PagenumberWordLengthAverage.Minimum }}
$PagenumberWordLengthAverage =($Global:BookPageNumber.word.string) | measure -Maximum -Minimum -Average -Property 'length'
#
}

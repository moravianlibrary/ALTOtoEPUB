...
#reset control vareable 
$isTITLE= 0

if ($FindTitle.word.string){
foreach ($word in ($FindTitle.Word)){

foreach ($character in (($word.string).ToCharArray()) )`
{
...
if ($UpperCharCount -ge ((($word.string).Length/100)*50)) { 
...
#write find TITLEs
 if ($isTITLE -eq ($FindTitle.Word).Length ){$Global:BookTitle += $FindTitle}
 }#foreach word end
}
